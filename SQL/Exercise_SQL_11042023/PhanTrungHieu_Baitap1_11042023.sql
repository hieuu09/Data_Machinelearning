...
-- Cách 2 
ALTER PROCEDURE C2_Bai2
	@_chuoi VARCHAR(100)
AS
BEGIN
		DECLARE @_chuoiisnull NVARCHAR(500) = '', @_Str NVARCHAR(500),  @_chuoi1 VARCHAR(100) = ''
	    IF OBJECT_ID('tempdb..#bang1') IS NOT NULL DROP TABLE #bang1
		IF (@_chuoi = '')
			BEGIN
				SELECT @_chuoi1 = @_chuoi1 +', '+makho FROM #bai2 GROUP BY makho
				SET @_chuoi1 = RIGHT(@_chuoi1, LEN(@_chuoi1)-1)
				SELECT @_chuoiisnull = @_chuoiisnull +', ISNULL([' + Makho+'], 0) AS '+makho+' ' FROM #bai2 GROUP BY Makho
			END
		ELSE
			BEGIN
				SELECT TRIM(VALUE) AS ds INTO #bang1 FROM STRING_SPLIT(@_chuoi,',')
...
ALTER PROCEDURE getRecordPage1
		...
		@_numberrecoder INT
AS BEGIN 
	DECLARE @_linestart INT, @_lineend INT
	SET @_linestart = (@_page * @_numberrecoder) - @_numberrecoder +1
	SET @_lineend = @_numberrecoder * @_page

	SELECT MaVT, TenVT 
	FROM ( SELECT MaVT, TenVT, ROW_NUMBER() OVER (ORDER BY CONVERT(INT, RIGHT(MaVT, LEN(MaVT) -2))) AS A FROM #bai4 ) AS B
	WHERE B.A BETWEEN @_linestart AND @_lineend
END
GO
-- trang số, số bản ghi 1 trang
EXEC getRecordPage1 1, 4
...
IF OBJECT_ID('tempdb..#bai6') IS NOT NULL DROP TABLE #bai6
CREATE TABLE #bai6
(
	CustomerID INT,
...
INSERT INTO #bai6(CustomerID, CustomerName, City, Street, So_nha) 
VALUES (1, N'Nguyễn Văn A', N'Hà Nội', N'Trần Duy HƯng', 1),
	(2, N'Lê Văn B', N'Hà Nội', N'Lê Dức Thọ', 2),
	(3, N'Trần Tiến Đạt', N'Nam Định', N'Trần HƯng Đạo', 25),
	(4, N'Nguyễn Tiến Trọn', N'Vĩnh Phúc', N'Đỗ Mười', 29),
...
CREATE PROCEDURE Oderby_Name
	@_choose INT
AS
BEGIN 
	IF @_choose =0 
		SELECT CustomerID, CustomerName, city, street, so_nha 
		FROM #bai6 ORDER BY  street ASC

	ELSE IF @_choose =1
	-- viết câu lệnh cùng thành phố
	SELECT CustomerID, CustomerName, city, street, so_nha 
	FROM #bai6 ORDER BY  City ASC
	
	ELSE SELECT 'Bạn nhập 0 sắp xếp theo street, 1 để sắp xếp theo city'
END
EXEC Oderby_name 1
...
INSERT INTO KK(Makhach, TenKhach, Diachi, Ncc, KK) 
VALUES ('KH02', 'Cong ty ABC', N'Hà Nội', 0,1)
...
BEGIN
	...
	IF ((SELECT NCC FROM INSERTED) = 1)
		INSERT INTO NCC (MaKhach, TenKhach, Diachi, Ncc) 
					SELECT INSERTED.MaKhach, INSERTED.TenKhach, INSERTED.Diachi, INSERTED.Ncc 
					FROM INSERTED;
END
